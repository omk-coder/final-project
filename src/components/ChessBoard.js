...
export function ChessBoard({ gameId, className, myAddr }) {
  ...
  function onMoveReceived(message) {
    ...
    if (message.type === 'game_over') {
      setIsGameOver(true);
      setWinner(message.winner);
      setLoser(myColor === 'white' ? 'black' : 'white');
    } else if (message.type === 'start_game') {
      const color = message.oppColor;
      if (myColor !== color) {
        setMyColor(color);
  ...
  function sendMove(message) {
    ...
    // const clientAddres = myAddr;
    // console.log('Winner Address', clientAddres);
  }
  //additional func you can delete it
  function sendMoveToLoser(message) {
    console.log('Message sent', message);
    gameChannel.publish({ name: 'game', data: message });
  }
  ...
  useEffect(() => {
        ...
        setPlayer1Time((prevTime) => {
          if (prevTime === 0) {
            ...
            const loserMessage = {
              type: 'game_over',
              winner: 'black',
              message: 'Oops! You lost the game!'
            };

            sendMove(message);
            sendMoveToLoser(loserMessage);
            return 0;
          }
          return prevTime - 1;
        });
    ...
    if (game.in_checkmate() || game.in_stalemate()) {
      let winnerTmp = ''; 
      let loserTmp = '';//write here about you lose
      if (game.in_checkmate()) {
        winnerTmp = game.turn() === 'w' ? 'black' : 'white';
        loserTmp = game.turn() === 'w' ? 'white' : 'black';
        setWinner(winnerTmp);
      }
      clearInterval(timer);
      sendMove({
  ...
  function handleQuit() {
    clearInterval(timer);
    setWinner(oppColor === 'white' ? 'black' : 'white');
    setLoser(oppColor);
    sendMove({
      type: 'game_over',
      winner: oppColor === 'white' ? 'black' : 'white',
    });
    setIsGameOver(true);
  }
