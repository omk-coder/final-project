...
export default function Sidebar({ children }) {
  ...
  return (
    <div className="flex h-full w-full text-white overflow-y-hidden">
      {/* Side Nav bar */}
      <div className="hidden flex-col bg-[#100303] md:flex md:min-w-[15%] justify-between py-16 px-8 border-solid border-0 border-r-2 border-[#AA69FF]">
        {/* Menu List */}
        <div className="flex flex-col space-y-4  ">
          {menuList.map((item, index) => (
            <Link
              className={`flex items-center py-3 px-5 gap-2 rounded-xl text-black no-underline transition duration-300  ${
            ...
            >
              <img
                ...
                className="w-5 "
              />
              <span className="font-manrope font-[600] text-sm text-white">
                {item.title}
              </span>
            </Link>
          ))}
        ...
        <div className="flex flex-col px-3 rounded-lg bg-lavender-300 border border-gray-500 border-solid shadow-lg shadow-gray-600 mt-5">
          ...
          <div className="flex py-1.5 justify-between items-center border-0 border-b border-gray-600 border-solid">
            ...
            <div className="flex items-center space-x-2">
              <div
                className={`w-8 h-8 rounded-full bg-gradient-to-br from-blue-600 to-red-600`}
              ></div>
